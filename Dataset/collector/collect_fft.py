...
def collect_one_fft(N=1024, batch=1, dtype=torch.float32):
    A = torch.randn(batch, N, dtype=dtype, device="cpu")
    ...
    return {
        "operation_type": "fft",
        "log_size": float(round(torch.log10(torch.tensor(N*batch+1.)).item(), 4)),
